...
class Vm:
    ...
    def call_value(self, callee, arg_count):
        ...
        if arg_count != callee.arity:
            return False
        self.frames[self.frames_top] = CallFrame(
            callee, 0, self.stack_top - arg_count - 1
        )
        self.frames_top += 1
        self.frame = self.frames[self.frames_top - 1]
        return True
    ...
    def interpret(self):
        ...
        while True:
            jitdriver.jit_merge_point(
                ip=self.frame.ip,
                frame_start=self.frame.frame_start,
                self=self
            )
            # stdin.readline(1024)
